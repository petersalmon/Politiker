...
$(document).ready(function(){
    ...
    var scriptTitles = $(".script-vals-hidden[data-type='phone']").map(function(){return $(this).attr("data-title");}).get();
...
$(document).on('click','#phoneTable tr',function(e){
...
function getStatCounts(){
    var numTrs = $("#phoneTable").children('tbody').children('tr').length;

    var contactedArray = $('#phoneTable td:nth-child(5) select').map(function(){
        return $(this).val();
    }).get()
    var contactedCounts = {};
    ...
    var respondedArray = $('#phoneTable td:nth-child(6) select').map(function(){
        return $(this).val();
    }).get()
    var respondedCounts = {};
    ...
    var supportArray = $('#phoneTable td:nth-child(7) select').map(function(){
        return $(this).val();
    }).get()
    var supportCounts = {};
    ...
    // console.log("suport counts: " + supportCounts);
    
    return [numTrs, numContacted, numResponded, supportCounts];
}

// highlight table row
$('#phoneTable').bind('click', function(e) {
    if($('#edit-mode-switch').is(':checked')) {
...
$("#save-camp-btn").click(function(){
    ...
    $("#phoneTable").find('tr').each(function(i) {
    ...
    customColDict= {};
    $("#phoneTable th.custom-col").each(function(){
        var colDict = {}
    ...
    customInputs.each(function(i){
        ...
        if(colName === "Called"){
            for(var id of Object.keys(idValMap)){
                ...
                $("td.ID").filter(function(){return $(this).text() === id;}).closest('tr').find('td:nth-child(5)').find('select').val(idValMap[id]);
            }
        }
        else if(colName === "Spoken To"){
            for(var id of Object.keys(idValMap)){
                // get row with specific id; go to the last cell; set the value to the value associated with the key
                $("td.ID").filter(function(){return $(this).text() === id;}).closest('tr').find('td:nth-child(6)').find('select').val(idValMap[id]);
            }
        }
        else if(colName === "Level of Support"){
            for(var id of Object.keys(idValMap)){
                // get row with specific id; go to the last cell; set the value to the value associated with the key
                $("td.ID").filter(function(){return $(this).text() === id;}).closest('tr').find('td:nth-child(7)').find('select').val(idValMap[id]);
            }
        }
        else{
            for(var id of Object.keys(idValMap)){
                // get row with specific id; go to the last cell; set the value to the value associated with the key
                $("td.ID").filter(function(){return $(this).text() === id;}).closest('tr').find('td:nth-child(8)').find('div').html(idValMap[id]);
            }
        }
    });
});

// dynamically create and update bottom stats upon page load
$(document).ready(function(){
...
// dynamically create and update bottom stats upon select change
$(document).on('change','#phoneTable select',function(){
    var counts = getStatCounts();
